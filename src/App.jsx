...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const stored = await window.storage.get("pharma_requests");
        if (stored) setRequests(JSON.parse(stored.value));
      } catch {}
    })();
  }, []);
  ...
  if (view === "home") return (
    <div style={{ minHeight:"100vh", background:"linear-gradient(135deg,#0F2027,#203A43,#2C5364)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", fontFamily:"'Georgia',serif", padding:"24px" }}>
      <div style={{ textAlign:"center", marginBottom:"44px" }}>
        ...
        <h1 style={{ color:"#E8D5A3", fontSize:"clamp(22px,5vw,34px)", fontWeight:"700", margin:"0 0 6px", letterSpacing:"0.04em" }}>Pharmacie</h1>
  ...
  if (view === "employee" && selectedEmployee) {
    ...
    const color = EMPLOYEE_COLORS[idx];
    const totalApproved = myRequests.filter(r => r.status === "approved").reduce((s, r) => s + r.days, 0);
    return (
          ...
          <div style={{ maxWidth:"600px", margin:"0 auto" }}>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:"14px" }}>
              ...
              <div>
                ...
                <div style={{ opacity:0.8, fontSize:"13px" }}>{totalApproved} jour{totalApproved>1?"s":""} approuvé{totalApproved>1?"s":""}</div>
              </div>
            </div>
          </div>
        </div>
          ...
          {activeTab === "submit" && (
            <div style={{ background:"white", borderRadius:"16px", padding:"24px", boxShadow:"0 2px 12px rgba(0,0,0,0.06)" }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"12px", marginBottom:"14px" }}>
                {[["startDate","Date de début"],["endDate","Date de fin"]].map(([field,label]) => (
                  <div key={field}>
              ...
              {form.startDate && form.endDate && form.endDate >= form.startDate && (
                <div style={{ background:"#EBF5FB", borderRadius:"8px", padding:"10px 14px", marginBottom:"14px", fontSize:"13px", color:"#2980B9", fontWeight:"600" }}>
                  📅 {getDaysBetween(form.startDate,form.endDate)} jour{getDaysBetween(form.startDate,form.endDate)>1?"s":""}
                </div>
              )}
                  ...
                  {[...myRequests].reverse().map(req => {
                    ...
                    return (
                      <div key={req.id} style={{ background:"white", borderRadius:"14px", padding:"16px", boxShadow:"0 2px 8px rgba(0,0,0,0.06)", borderLeft:`4px solid ${s.border}` }}>
                        ...
                        <div style={{ color:"#6B8A99", fontSize:"12px", marginBottom:"4px" }}>📅 {formatDate(req.startDate)} → {formatDate(req.endDate)} · <strong>{req.days} jour{req.days>1?"s":""}</strong></div>
    ...
    <div style={{ minHeight:"100vh", background:"#F5F3EF", fontFamily:"'Georgia',serif" }}>
      <div style={{ background:"linear-gradient(135deg,#1a2e38,#2C5364)", padding:"18px 24px", color:"white" }}>
        <div style={{ maxWidth:"900px", margin:"0 auto", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            <h1 style={{ margin:0, fontSize:"18px", color:"#E8D5A3" }}>💊 Espace Gérant</h1>
            <p style={{ margin:"3px 0 0", fontSize:"12px", color:"#94B4C8" }}>Pharmacie – Gestion des congés</p>
          </div>
      ...
      <div style={{ maxWidth:"900px", margin:"0 auto", padding:"24px" }}>
        ...
        {/* Requests */}
        <div style={{ background:"white", borderRadius:"16px", padding:"22px", marginBottom:"24px", boxShadow:"0 2px 12px rgba(0,0,0,0.06)" }}>
                  ...
                  return (
                    <div key={req.id} style={{ border:"1px solid #E8E4DC", borderRadius:"12px", padding:"14px 16px", display:"flex", flexWrap:"wrap", gap:"10px", alignItems:"center" }}>
                      ...
                      <div style={{ flex:1, minWidth:"120px" }}>
                        ...
                        <div style={{ color:"#6B8A99", fontSize:"12px" }}>{req.type} · {req.days} jour{req.days>1?"s":""}</div>
        ...
        <div style={{ background:"white", borderRadius:"16px", padding:"22px", marginBottom:"24px", boxShadow:"0 2px 12px rgba(0,0,0,0.06)" }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"18px", flexWrap:"wrap", gap:"10px" }}>
            <h3 style={{ margin:0, color:"#1a2e38", fontSize:"17px" }}>Calendrier des absences approuvées</h3>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:"3px" }}>
            ...
            {Array(daysInMonth).fill(null).map((_,i) => {
              ...
              return (
                <div key={day} style={{ minHeight:"48px", padding:"3px", borderRadius:"7px", background:isToday?"#EBF5FB":absences.length>0?"#FFF9F0":"#F9F8F6", border:isToday?"1px solid #2980B9":"1px solid transparent" }}>
        ...
        {/* Password + email reference */}
        <div style={{ background:"white", borderRadius:"16px", padding:"20px", boxShadow:"0 2px 12px rgba(0,0,0,0.06)" }}>
          <h3 style={{ margin:"0 0 14px", color:"#1a2e38", fontSize:"16px" }}>👥 Équipe</h3>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))", gap:"8px" }}>
            {EMPLOYEES.map((emp, i) => (
              <div key={emp.name} style={{ background:"#F9F8F6", borderRadius:"10px", padding:"10px 14px", display:"flex", alignItems:"center", gap:"10px" }}>
                <div style={{ width:"34px", height:"34px", borderRadius:"50%", background:EMPLOYEE_COLORS[i], display:"flex", alignItems:"center", justifyContent:"center", color:"white", fontSize:"12px", fontWeight:"700", flexShrink:0 }}>
                  {initials(emp.name)}
                </div>
                <div style={{ overflow:"hidden" }}>
                  <div style={{ fontWeight:"700", fontSize:"13px", color:"#1a2e38" }}>{emp.name}</div>
                  <div style={{ fontSize:"11px", color:"#94B4C8", fontFamily:"monospace" }}>{emp.password}</div>
                  <div style={{ fontSize:"10px", color:"#B0C4CE", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>✉️ {emp.email}</div>
                </div>
              </div>
            ))}
          </div>
